...
use govrix_ai_oss_store::EventFilter;
...
pub async fn list_events(
    ...
    match govrix_ai_oss_store::events::list_events(&state.pool, &filter).await {
        Ok(events) => {
...
pub async fn get_event(
    ...
    match govrix_ai_oss_store::events::get_event(&state.pool, uuid).await {
...
pub async fn get_session_events(
    ...
    match govrix_ai_oss_store::events::get_session_events(&state.pool, uuid).await {
        Ok(events) => {
            let total = events.len();
            (
                StatusCode::OK,
