//! Event channel and background writer for Govrix AI OSS proxy.
//!
//! Architecture:
...
use govrix_ai_oss_common::models::agent::{Agent, AgentType};
use govrix_ai_oss_common::models::event::AgentEvent;
...
async fn flush_batch(
    ...
    match pool {
        Some(p) => {
            match govrix_ai_oss_store::events::insert_events_batch(p, batch).await {
                Ok(count) => {
...
async fn upsert_agents_from_batch(
    pool: &govrix_ai_oss_store::StorePool,
    batch: &[AgentEvent],
    metrics: &Arc<Metrics>,
    seen_agents: &mut std::collections::HashSet<String>,
) {
    ...
    for (agent_id, accum) in &agents {
        ...
        if let Err(e) = govrix_ai_oss_store::agents::upsert_agent(pool, &agent).await {
            tracing::warn!(
                error = %e,
                agent_id = %agent_id,
...
mod tests {
    ...
    fn event_channel_try_send_non_blocking() {
        ...
        rt.block_on(async {
            ...
            let event = govrix_ai_oss_common::models::event::AgentEvent::new(
                "agent-1",
                uuid::Uuid::now_v7(),
                govrix_ai_oss_common::models::event::EventDirection::Outbound,
                "POST",
                "https://api.openai.com/v1/chat/completions",
                govrix_ai_oss_common::models::event::Provider::OpenAI,
                "genesis",
                "audit:none",
            );
