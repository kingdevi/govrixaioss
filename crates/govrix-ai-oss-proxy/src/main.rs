//! Govrix AI OSS proxy binary entry point.
//!
...
use govrix_ai_oss_proxy::{api, events, proxy};
...
async fn main() -> anyhow::Result<()> {
    ...
    let pool_result = govrix_ai_oss_store::connect(&config.database).await;
    ...
    let policy_engine = {
        ...
        if path.exists() {
            match govrix_ai_oss_proxy::policy::PolicyEngine::from_file(path) {
                Ok(engine) => {
                ...
                Err(e) => {
                    ...
                    govrix_ai_oss_proxy::policy::PolicyEngine::noop()
                }
            }
        } else {
    ...
    let policy_hook: std::sync::Arc<dyn govrix_ai_oss_proxy::policy::PolicyHook> =
