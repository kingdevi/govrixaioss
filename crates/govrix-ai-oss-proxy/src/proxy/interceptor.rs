...
use govrix_ai_oss_common::models::event::{AgentEvent, EventDirection, Provider};
use govrix_ai_oss_common::protocols::Protocol;
...
pub struct InterceptorState {
    ...
    pub db_pool: Option<govrix_ai_oss_store::StorePool>,
}
...
impl InterceptorState {
    ...
    pub fn with_pool_and_upstream_urls(
        ...
        policy_hook: Arc<dyn PolicyHook>,
        upstream_urls: UpstreamUrls,
        db_pool: govrix_ai_oss_store::StorePool,
    ) -> Self {
        Self {
            session_tracker: Mutex::new(SessionTracker::new()),
            event_sender,
...
pub async fn log_request_event(ctx: &RequestContext, state: &InterceptorState) {
    ...
    match &ctx.protocol {
        Protocol::OpenAI { .. } => {
            if let Some(req) =
                govrix_ai_oss_common::protocols::openai::parse_request(&ctx.request_body)
            {
        ...
        Protocol::Anthropic { .. } => {
            if let Some(req) =
                govrix_ai_oss_common::protocols::anthropic::parse_request(&ctx.request_body)
            {
                event.model = Some(req.model);
            }
        }
...
pub async fn log_response_event(
    ...
    match &ctx.protocol {
        Protocol::OpenAI { streaming, .. } => {
            if *streaming {
                ...
                let acc =
                    govrix_ai_oss_common::protocols::openai::parse_streaming_chunks(&data_lines);
                event.model = acc.model.or_else(|| {
                    govrix_ai_oss_common::protocols::openai::parse_model(&ctx.request_body)
                });
                ...
                govrix_ai_oss_common::protocols::openai::parse_response(response_body)
            {
        ...
        Protocol::Anthropic { streaming, .. } => {
            if *streaming {
                ...
                let streaming_acc =
                    govrix_ai_oss_common::protocols::anthropic::accumulate_streaming_events(
                        &acc_state.anthropic_events,
                    );
                event.model = streaming_acc.model.clone().or_else(|| {
                    govrix_ai_oss_common::protocols::anthropic::parse_model(&ctx.request_body)
                });
                ...
                govrix_ai_oss_common::protocols::anthropic::parse_response(response_body)
            {
                event.model = Some(resp.model);
    ...
    if let (Some(input), Some(output)) = (event.input_tokens, event.output_tokens) {
        if let Some(ref model_name) = event.model {
            if let Some(pricing) = govrix_ai_oss_common::models::pricing::lookup_pricing(model_name)
            {
...
pub async fn analyze_request(
    ...
    match protocol {
        Protocol::OpenAI { .. } => {
            let model = govrix_ai_oss_common::protocols::openai::parse_model(body);
            tracing::debug!(
                agent = agent_id,
                model = ?model,
                bytes = body.len(),
        ...
        Protocol::Anthropic { .. } => {
            let model = govrix_ai_oss_common::protocols::anthropic::parse_model(body);
            tracing::debug!(
                agent = agent_id,
                model = ?model,
                bytes = body.len(),
        ...
        Protocol::Mcp { server, .. } => {
            if let Some(req) = govrix_ai_oss_common::protocols::mcp::parse_request(body) {
                let tool = govrix_ai_oss_common::protocols::mcp::extract_tool_name(&req);
                tracing::debug!(
                    agent = agent_id,
                    server = server,
...
pub async fn analyze_response(
    ...
    match protocol {
        Protocol::OpenAI { .. } => {
            let _usage = govrix_ai_oss_common::protocols::openai::parse_usage(body);
        }
        Protocol::Anthropic { .. } => {
            let _usage = govrix_ai_oss_common::protocols::anthropic::parse_usage(body);
        }
        _ => {}
    }
