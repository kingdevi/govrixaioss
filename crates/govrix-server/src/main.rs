...
//!
//! Embeds the Govrix AI OSS proxy engine and adds:
...
//!
//! Startup: loads Platform config, validates license, initializes OSS proxy.
...
/// Forward a request arriving on the mTLS TLS listener to the plain-HTTP OSS proxy.
///
/// This implements TLS termination: Enterprise agents connect via TLS/mTLS on port 4443,
/// and their requests are forwarded to the OSS proxy (port 4000) which handles policy
...
use govrix_ai_oss_common::config::Config;
use govrix_ai_oss_proxy::{api as oss_api, events, policy::PolicyHook, proxy};
...
async fn main() -> anyhow::Result<()> {
    ...
    // ── Database pool ───────────────────────────────────────────────────────
    let pool = match govrix_ai_oss_store::connect(&config.database).await {
        Ok(p) => {
    ...
    if let Some(tls_config) = mtls_tls_config {
        ...
        // Forward mTLS traffic to the OSS proxy on its plain-HTTP port.
        // This is a TLS termination proxy: agents with mTLS certs connect here,
        // their requests pass through the same policy enforcement at the OSS proxy.
        let inner_proxy_port = config.proxy.port;
        tracing::info!(
            addr = %mtls_addr,
            inner_port = inner_proxy_port,
            "mTLS TLS proxy starting (forwarding to OSS proxy)"
        );
    ...
    let api_handle = tokio::spawn(async move {
        let result = match pool {
            Some(p) => {
                oss_api::serve_with_pool_and_routes(
                    api_addr,
                    p,
                    api_config,
                    api_metrics,
            ...
            None => oss_api::serve(api_addr).await,
        };
        if let Err(e) = result {
